...
#include "GT_902PMGG_blocking.hpp"
...
//GPSを受信
void GPS::receive(){
    for(int i=0; i<read_len; i++) recv[i] = 0;
    for(int i=0; i<50; i++){
        if(uart_is_readable(uart_hw)){
            uart_read_blocking(uart_hw, (uint8_t *) &recv, read_len);
            break;
        }
        sleep_ms(10);
    }
...
GPS::Measurement_t GPS::measure(){
    ...
    while(i < read_len){
        ...
        while(i<read_len && recv[i++]!='$');  //最初の$まで移動
        for(; i<read_len && recv[i]!='*'; i++){ //*になるまで次の文字を見ていく
            check_sum ^= recv[i];
            if(recv[i] == ','){
                split_data.push_back("");
                continue;
            }
            split_data.back() += recv[i];
        }
        ...
        i++;
        while(i<read_len && recv[i]!='\r' && recv[i]!='\n'){
            check_sum_str += recv[i++];
        }
